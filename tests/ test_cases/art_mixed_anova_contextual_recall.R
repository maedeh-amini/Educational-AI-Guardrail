# ==============================================================================
# ART_Mixed_ANOVA CONTEXTUAL RECALL 
# ==============================================================================
...
# 3: Load the data
data_ContextualRecall <- read.csv(choose.files(), header=T)

# Convert categorical variables to factors
data_ContextualRecall$Architecture <- as.factor(data_ContextualRecall$Architecture)
data_ContextualRecall$DataType <- as.factor(data_ContextualRecall$DataType)
data_ContextualRecall$Question_ID <- as.factor(data_ContextualRecall$Question_ID)
str(data_ContextualRecall)

# ==============================================================================
# PHASE 1: EXPLORATORY DATA ANALYSIS (SCORE)
# ==============================================================================

print("--- PHASE 1: Descriptive Statistics for Contextual Recall Score ---")
# Calculate descriptive statistics using dplyr
score_summary <- data_ContextualRecall %>%
...
plot_score_hist_curve <- ggplot(data_ContextualRecall, aes(x = Score, fill = DataType, color = DataType)) +
  geom_histogram(aes(y = after_stat(density)), binwidth = 0.05, position = "identity", alpha = 0.4) +
  geom_density(alpha = 0.2, linewidth = 1) +
  facet_wrap(~ Architecture) +
  labs(title = "Distribution of Contextual Recall Scores with Density Curves", 
       x = "Score", 
       y = "Density") +
  theme_minimal()
...
plot_score_box <- ggplot(data_ContextualRecall, aes(x = Architecture, y = Score, fill = DataType)) +
  geom_boxplot(alpha = 0.7) +
  labs(title = "Boxplot of Contextual Recall Scores", x = "Architecture", y = "Score") +
  theme_minimal()
...
basic_lm <- lm(Score ~ Architecture * DataType, data = data_ContextualRecall)
...
levene_result <- leveneTest(Score ~ Architecture * DataType, data = data_ContextualRecall)
...
art_model <- art(Score ~ Architecture * DataType + (1|Question_ID), data = data_ContextualRecall)
...
plot_score_bar <- ggplot(data_ContextualRecall, aes(x = Architecture, y = Score, fill = DataType)) +
  stat_summary(fun = mean, geom = "bar", position = "dodge") +
  labs(title = "Mean Contextual Recall by Architecture and Data Type",
       x = "RAG Architecture", y = "Mean Contextual Recall Score") +
  theme_minimal()
...
data_ContextualRecall$Error_Rate <- 1 - data_ContextualRecall$Score

error_summary <- data_ContextualRecall %>%
  group_by(Architecture, DataType) %>%
  dplyr::summarise(
    N = dplyr::n(),
...
print("--- PHASE 4: Contextual Recall Error Rate Summary with Confidence Intervals ---")
print(error_summary)

plot_error_hist_curve <- ggplot(data_ContextualRecall, aes(x = Error_Rate, fill = DataType, color = DataType)) +
  geom_histogram(aes(y = after_stat(density)), binwidth = 0.05, position = "identity", alpha = 0.4) +
  geom_density(alpha = 0.2, linewidth = 1) +
  facet_wrap(~ Architecture) +
  labs(title = "Distribution of Contextual Recall Error Rates with Density Curves", 
       x = "Error Rate", 
       y = "Density") +
  theme_minimal()
print(plot_error_hist_curve)

plot_error_box <- ggplot(data_ContextualRecall, aes(x = Architecture, y = Error_Rate, fill = DataType)) +
  geom_boxplot(alpha = 0.7) +
  labs(title = "Box Plot of Contextual Recall Error Rates", x = "RAG Architecture", y = "Error Rate") +
  theme_minimal()
print(plot_error_box)

plot_raincloud <- ggplot(data_ContextualRecall, aes(x = Architecture, y = Error_Rate, fill = DataType)) +
  ...
  labs(title = "Contextual Recall Error Rates: Raincloud Distribution",
       x = "RAG Architecture", y = "Error Rate") +
  theme_classic(base_size = 14) +
  theme(legend.position = "top")
print(plot_raincloud)

plot_violin_box <- ggplot(data_ContextualRecall, aes(x = Architecture, y = Error_Rate, fill = DataType)) +
  ...
  scale_fill_viridis_d(option = "mako", end = 0.8) +
  labs(title = "Contextual Recall Error Rates: Violin & Boxplot",
       x = "RAG Architecture", y = "Error Rate") +
  theme_classic(base_size = 14)
...
plot_interaction_ci <- ggplot(error_summary, aes(x = DataType, y = Mean_Error, group = Architecture, color = Architecture)) +
  ...
  labs(title = "Interaction Effect: Contextual Recall Domain Shift",
...
output_dir <- "E:/THSI/Code/statistical_analyses _R/data and results/converted_data/contextual_recall"

# 1. Save core summaries and effect sizes as CSV files
write.csv(score_summary, file.path(output_dir, "ContextualRecall_Score_Summary.csv"), row.names = FALSE)
write.csv(eta_table, file.path(output_dir, "ContextualRecall_ANOVA_EtaSquared.csv"), row.names = FALSE)
write.csv(g_table, file.path(output_dir, "ContextualRecall_Hedges_G.csv"), row.names = FALSE)
write.csv(error_summary, file.path(output_dir, "ContextualRecall_Error_Summary_CI.csv"), row.names = FALSE)

# 2. Save quantified model-based differences as CSV files
write.csv(datatype_diff_table, file.path(output_dir, "ContextualRecall_DataType_Differences.csv"), row.names = FALSE)
write.csv(arch_diff_table, file.path(output_dir, "ContextualRecall_Architecture_Differences.csv"), row.names = FALSE)
write.csv(interaction_diff_table, file.path(output_dir, "ContextualRecall_Interaction_Contrasts_Differences.csv"), row.names = FALSE)

# 3. Save raw mean differences as CSV files
write.csv(raw_diffs_datatype, file.path(output_dir, "ContextualRecall_Raw_Diffs_DataType.csv"), row.names = FALSE)
write.csv(raw_diffs_architecture, file.path(output_dir, "ContextualRecall_Raw_Diffs_Architecture.csv"), row.names = FALSE)

# 4. Sink raw statistical structures directly to the text file report
sink(file.path(output_dir, "ContextualRecall_Full_Statistical_Report.txt"))
shapiro_result
levene_result
art_anova_results
summary(interaction_contrasts)
